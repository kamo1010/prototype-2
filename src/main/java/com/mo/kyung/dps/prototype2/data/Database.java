...
import com.mo.kyung.dps.prototype2.exceptions.MissingEntryInDatabaseException;
...
//		getUser("jerome").subscribeToTopic(getTopic("Karren"));
	...
	public static AccountUser getUser(String login) throws MissingEntryInDatabaseException {
		for (AccountUser user : users) {
			if (user.getLogin().equals(login)) {
				return user;
			}
		}
		throw new MissingEntryInDatabaseException(new StringBuilder("Login ").append(login).append(" does not exist").toString());
	}
	public static boolean addUser(AccountUser user) throws MissingEntryInDatabaseException {
		if (users.add(user)) {
	...
	public static Topic getTopic(String topicName) throws MissingEntryInDatabaseException {
		for (Topic topic : topics) {
			if (topic.getName().equals(topicName)) {
				return topic;
			}
		}
		throw new MissingEntryInDatabaseException(new StringBuilder("Topic ").append(topicName).append(" does not exist").toString());
	}
